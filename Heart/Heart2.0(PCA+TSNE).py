...
np.random.seed(77)
cv = cross_val_score(CatBoostClassifier(verbose=0), X, y, cv=5, scoring='accuracy')
score = np.mean(cv)
print('The cross-val score of CatBoost is : ', score)
...
df_test = pd.DataFrame(X_test, columns=['PC1', 'PC2'])
df_test['Index'] = y_test
df_test['prediction'] = predicted2

sns.lmplot('PC1', 'PC2', hue='Index', data=df_test, fit_reg=False)
plt.title('stacking (test--real)')
plt.show()
sns.lmplot('PC1', 'PC2', hue='prediction', data=df_test, fit_reg=False)
plt.title('stacking (test--prediction)')
plt.show()
